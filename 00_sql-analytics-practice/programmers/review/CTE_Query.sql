...
| 함수           | 동점 처리          | 다음 순위 처리     | 예시 결과 (점수: 100, 95, 95, 90) | 주요 사용처                    |
| ------------ | -------------- | ------------ | --------------------------- | ------------------------- |
| ROW_NUMBER() | 동점이어도 다른 순번 부여 | 건너뜀 없음       | 1, 2, 3, 4                  | 중복 제거, 페이징, 그룹 내 1개 선택    |
| RANK()       | 동점이면 같은 순위     | 다음 순위 건너뜀    | 1, 2, 2, 4                  | 실제 순위 계산 (시험 등수, 매출 순위 등) |
| DENSE_RANK() | 동점이면 같은 순위     | 다음 순위 건너뜀 없음 | 1, 2, 2, 3                  | 순위 그룹 유지하면서 연속 순위 필요할 때   |

- CTE는 실제 임시 테이블이 아니라 쿼리 내부에서 사용하는 이름 붙은 서브쿼리이다.
- 대부분의 DB는 CTE를 별도로 생성하지 않고 전체 쿼리로 합쳐 옵티마이저가 재작성(inline)한다.
- 따라서 필터, JOIN, 조건 등이 CTE 내부로 밀려 들어가거나(pushdown) 실행 순서가 변경될 수 있다.
